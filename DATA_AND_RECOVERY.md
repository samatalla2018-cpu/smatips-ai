...
| **Subscription** (legacy, `subscriptions` table) | 1:1 with an Account | Same moment as the Account (`INSERT OR IGNORE` into `subscriptions`, `status='pending'`) | `phone` (PRIMARY KEY, 1:1 with `users`) | **Superseded by per-trip payment (migration `0003_trip_payments.sql`, "Convert SmaTrips AI to pay-per-trip").** The product is no longer a subscription: each trip (`trip_id`) requires its own SAR 49 payment (see `Trip Files` row below and [functions/api/payment/create.js](functions/api/payment/create.js)). The `subscriptions` table and `phone`-only webhook fallback ([functions/api/payment/webhook.js](functions/api/payment/webhook.js)) are kept only for any in-flight legacy invoice created before the conversion — every new invoice always carries `trip_id`. No deletion path exists for this legacy row either. |
...
| **Trip Files** (`trips` row) | The Account that exported them | User explicitly exports/downloads a trip via [functions/api/trips/index.js](functions/api/trips/index.js) `onRequestPost` — each trip gets its own `id` and starts `payment_status='pending'` | `id` (UUID PK) + `phone` (owner, enforced on every read — [functions/api/trips/[id].js](functions/api/trips/[id].js)) | No deletion endpoint exists today. Documented gap. Reading/downloading a trip and using "رتّب لي اليوم من جديد" both require `payment_status='paid'` for **that specific `trip_id`** ([functions/_utils.js](functions/_utils.js) `isTripUnlocked`) — paying for one trip never unlocks another (verified by `tests/authorization.test.mjs` and `tests/payment.test.mjs`). |
