...
| `0004_bank_transfer.sql` | Additive-only: adds `bank_transfer_requests` (a second, manually-reviewed payment method alongside Moyasar). Each request is tied to one `trip_id`; a partial unique index blocks more than one active (`pending`/`under_review`) request per trip. Approval reuses the existing `activateTripPayment()` — no separate "trip pass" table. |
...
wrangler d1 execute smatripsai-db --remote --file=migrations/0004_bank_transfer.sql
```
...
## Planned (not yet applied) — 0005: constraints on existing tables
...
live user data than an additive `CREATE TABLE IF NOT EXISTS`). A future `0005_constraints.sql` is
planned to add:
